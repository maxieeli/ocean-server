...
use crate::{
    ...
    Item, MxCodecError, MxCodecResult,
};
...
impl YType {
    ...
    pub fn set_kind(&mut self, kind: YTypeKind) -> MxCodecResult {
        ...
        if self.kind() != kind {
            if self.kind == YTypeKind::Unknown {
                ...
                return Err(MxCodecError::TypeCastError(kind.as_str()));
            }
        }
        Ok(())
    }
...
impl YTypeBuilder {
    ...
    pub fn build_exists<T: TryFrom<YTypeRef, Error = MxCodecError>>(self) -> MxCodecResult<T> {
        let store = self.store.read().unwrap();
        let ty = if let Some(root_name) = self.root_name {
            match store.types.get(&root_name) {
                Some(ty) => ty.clone(),
                None => {
                    return Err(MxCodecError::RootStructNotFound(root_name));
                }
            }
        } else {
            return Err(MxCodecError::TypeCastError("root_name is not set"));
        };
        drop(store);
        T::try_from(ty)
    }
    pub fn build<T: TryFrom<YTypeRef, Error = MxCodecError>>(self) -> MxCodecResult<T> {
            ...
            fn try_from(value: super::YTypeRef) -> Result<Self, Self::Error> {
                if let Some((_, mut inner)) = value.write() {
                    match inner.kind {
                        ...
                        _ => Err($crate::MxCodecError::TypeCastError(std::stringify!($name))),
                    }
                } else {
                    Err($crate::MxCodecError::TypeCastError(std::stringify!($name)))
                }
            }
        }
        impl $name {
