...
impl ItemPosition {
    ...
    /// ---------------------------------
    ///    ^left   ^right
    pub fn normalize(&mut self, store: &mut DocStore) -> MxCodecResult {
        if self.offset > 0 {
...
pub(crate) trait ListType: AsInner<Inner = YTypeRef> {
    ...
    fn insert_after(ty: &mut YType, store: &mut DocStore, pos: ItemPosition, content: Content) -> MxCodecResult {
    ...
    fn remove_after(ty: &mut YType, store: &mut DocStore, mut pos: ItemPosition, len: u64) -> MxCodecResult {
        pos.normalize(store)?;
        ...
        if let Some(markers) = &ty.markers {
            markers.update_marker.changes(pos.index, -((len - remaining) as i64));
        }
        Ok(())
    }
}
